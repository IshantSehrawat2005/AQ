...
.coll-card[data-tier="Tier 1"]::before { background:linear-gradient(90deg,var(--accent2),var(--accent)); }
.coll-card[data-tier="Tier 2"]::before { background:linear-gradient(90deg,var(--accent),var(--accent3)); }
.coll-card[data-tier="Tier 3"]::before { background:linear-gradient(90deg,var(--accent3),var(--danger)); }
.coll-card:hover { transform:translateY(-3px); border-color:rgba(110,231,183,0.2); box-shadow:0 12px 40px rgba(0,0,0,0.3); }
.coll-header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px; }
.coll-name { font-family:'Syne',sans-serif; font-weight:700; font-size:1rem; flex:1; }
...
.cs-val { font-family:'Syne',sans-serif; font-weight:700; font-size:1rem; color:var(--accent); margin-top:2px; }
...
<main class="main">
  ...
  ?>
    <div class="coll-card" data-tier="<?= $c['tier'] ?>" data-name="<?= strtolower($c['college_name']) ?>" data-loc="<?= strtolower($c['location']) ?>" data-courses="<?= strtolower($c['course_list']) ?>">
...
function filterCards() {
  ...
  document.querySelectorAll('.coll-card').forEach(c=>{
    ...
    const textMatch = !q || c.dataset.name.includes(q) || c.dataset.loc.includes(q) || c.dataset.courses.includes(q);
